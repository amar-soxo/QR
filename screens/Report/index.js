    ...
    return (
        <View style={styles.container}>
            ...
            <SafeAreaView style={styles.safeArea}>
                ...
                <Card style={{ marginBottom: 10, shadowColor: 'black', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.8, shadowRadius: 1, marginHorizontal: 20 }}>
                    <View style={{ flexDirection: 'row' }}>
                        <View style={{ flexDirection: 'row' }}>
                            <KittenText style={{ fontSize: 15, fontWeight: 'bold' }} category='h1'>Patient ID :</KittenText>
                            <KittenText style={{ marginTop: 3 }} category='label'>{"   "}{patnt.Id}</KittenText>
                        </View>
                        <View style={{ flexDirection: 'row' }}>
                            <KittenText style={{ fontSize: 15, fontWeight: 'bold' }} category='h1'>{"         "}Status :</KittenText>
                            <KittenText style={{ marginTop: 3 }} category='label'>{"   "}{!isLoading && subEvents[subEvents.length - 1].EventStatus.Desc}</KittenText>
                        </View>
                    </View>
                    <View style={{ flexDirection: 'row' }}>
                        <KittenText style={{ fontSize: 15, fontWeight: 'bold' }} category='h1'>Gender :</KittenText>
                        <KittenText style={{ marginTop: 3 }} category='label'>{"   "}{patnt.Gender}</KittenText>
                    </View>
                    <View style={{ flexDirection: 'row' }}>
                        <KittenText style={{ fontSize: 15, fontWeight: 'bold' }} category='h1'>Event :</KittenText>
                        <KittenText style={{ marginTop: 3 }} category='label'>{"   "}{!isLoading && subEvents[subEvents.length - 1].Subevent.Desc}</KittenText>
                    </View>
                        ...
                        <View style={styles.timeLineView}>
                            ...
                            <Timeline
                                data={locations.map((item, index) => {
                                    const subEvent = subEvents.find(ele => ele.Location.Id == item.Id);
                                    let desc = subEvent
                                        ? `Start: ${
                                        subEvent
